...
const KB = {
  ...
  // -- KB: R data preparation
  r_prep: {
    title: "Preparing Your PyRate Input in R",
    plain: `Before running PyRate, you need to convert your fossil occurrence data into PyRate's input format using a short R script. The R function reads your spreadsheet, randomly samples fossil ages within their uncertainty ranges, and writes the *_PyRate.py input file that PyRate reads.`,
    technical: `# After running the R command, verify output and check for typos:
python PyRate.py -check_names path/to/*_TaxonList.txt
# → *_TaxonList_scores.txt  (ranks 0-1 = likely misspelling, 2-3 = different name variant)

# Get summary statistics of your dataset:
python PyRate.py path/to/*_PyRate.py -data_info | tee data_summary.txt

# Output files generated by R:
# *_PyRate.py     = PyRate-formatted input with replicated datasets
# *_TaxonList.txt = list of all taxa and their extant/extinct status
# *.txt           = copy of your original data as .txt (PBDB path only)

# Inside *_PyRate.py:
# - data_1 = first replicate, data_2 = second, etc.
# - Each dataset is a list of arrays, one array per species
# - Each number in an array = one fossil occurrence age (in Ma), randomly
#   sampled from within the fossil's min/max age range
# - Extant species have 0 as their last value (representing the present day)
# - taxa_names at the end lists which array belongs to which species`
  },
...
const TREE = {
  ...
  data_replicates: {
    ...
    options: [
      { label: "Create 10 replicate datasets (standard, recommended)", next: "r_prep", tags: ["rep10"], icon: "✓" },
      { label: "I want a different number of replicates", next: "r_prep", tags: ["rep_custom"], icon: "🔢" },
      { label: "Just one dataset, no replicates (simpler but ignores age uncertainty)", next: "r_prep", tags: ["rep1"], icon: "1️⃣" },
    ]
  },

  // -- Section: R data preparation command
  r_prep: {
    id: "r_prep", topic: "r_prep",
    r_cmd: true,
    question: "Generating your PyRate input file in R",
    subtitle: "Run the command below in an R notebook or RStudio. When your *_PyRate.py file has been created, continue to the next step.",
    explain: `Your input file should have **one row per fossil occurrence**. If the same species was found in five different rock layers, that is five rows — one per find, each with its own age range.
...
After running the R command you will have:
- **\`*_PyRate.py\`** — the PyRate input file with your replicated datasets. Each replicate is a list of arrays; each array is one species; each number is one fossil occurrence age (in Ma). Extant species have 0 as their last value, representing the present day.
- **\`*_TaxonList.txt\`** — a species list with extant/extinct status. Run a spelling check on this (see "technical details" below).`,
    options: [
      { label: "My *_PyRate.py file is ready — continue to model selection", next: "goal", tags: [], icon: "→" }
    ]
  },
...
function RCmdBuilder({ tags, T }) {
  ...
  const buildCmd = () => {
    const isPBDB = tags.includes("pbdb");
    const isSite = tags.includes("site_data");
    const hasExtant = tags.includes("has_extant");
    const rep10 = tags.includes("rep10");
    const repCustom = tags.includes("rep_custom");
    ...
    L.push('source("path/to/PyRate-master/pyrate_utilities.r")');
    L.push("");

    if (isPBDB) {
      if (hasExtant) {
        L.push("# List every species in your group that is still alive today");
        L.push('extant_species <- c("Species name 1", "Species name 2")  # ← Fill in your extant species');
        L.push("");
      }
      L.push("# Convert PBDB download to PyRate input format");
      L.push("extract.ages.pbdb(");
      L.push('  file = "path/to/your_pbdb_data.csv",     # ← Path to your downloaded PBDB CSV');
      if (hasExtant) L.push("  extant_species = extant_species,          # Marks these species as still living");
      if (repArg) L.push(repArg);
      L.push(")");
    } else {
      if (isSite) {
        L.push("# Convert site-based occurrence data to PyRate input format");
        L.push("# Ages are randomized by site, not by individual fossil occurrence");
      } else {
        L.push("# Convert your fossil occurrence spreadsheet to PyRate input format");
      }
      L.push("extract.ages(");
      L.push('  file = "path/to/your_data.txt",           # ← Path to your tab-delimited .txt file');
      if (repArg) L.push(repArg);
      L.push(")");
    }

    L.push("");
    L.push("# Optional: exclude fossils with very uncertain ages");
    L.push("# Add:  cutoff = 10  inside the call above to drop occurrences whose");
    L.push("# max_age - min_age is greater than 10 Ma (or whatever threshold you choose)");
